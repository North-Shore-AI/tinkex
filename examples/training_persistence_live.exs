defmodule Tinkex.Examples.TrainingPersistenceLive do
  ...
  def run do
    ...
    IO.puts("Checkpoint TTL: #{@checkpoint_ttl_seconds}s")
    ...
    with {:ok, service} <- Tinkex.ServiceClient.start_link(config: config),
         ...
         {:ok, save_task} <-
           Tinkex.TrainingClient.save_state(
             training,
             checkpoint_name,
         ...
         {:ok, restored} <-
           Tinkex.ServiceClient.create_training_client_from_state_with_optimizer(service, path) do
      IO.puts("Created a fresh training client with optimizer state: #{inspect(restored)}")
