defmodule Tinkex.Domain.Rest.Client do
  @moduledoc """
  ...
  defstruct [:session_id, :config, :context]
  ...
  """
  @spec new(String.t(), Config.t() | Context.t()) :: t()
  def new(session_id, %Context{config: %Config{} = config} = context) do
    %__MODULE__{session_id: session_id, config: config, context: context}
  end

  def new(session_id, %Config{} = config) do
    context = ContextBuilder.new(config)
    %__MODULE__{session_id: session_id, config: config, context: context}
  end
